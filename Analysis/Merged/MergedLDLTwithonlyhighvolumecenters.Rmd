---
title: "MergedLDLTwithonlyhighvolumecenters"
output:
...
```{r}
#bound2 <- bound2 %>% filter(TRANSPLANT_UNIT == "1" | TRANSPLANT_UNIT == "B" | TRANSPLANT_UNIT == "G" | TRANSPLANT_UNIT == "00124" | TRANSPLANT_UNIT == "00279" | TRANSPLANT_UNIT == "06200" | TRANSPLANT_UNIT == "21080" | TRANSPLANT_UNIT == "24304")
...
```
**OS overall DDLT vs LDLT CAN Fig 4b**
...
```{r Table 3 recipient and donor}
cox2 <- coxph(Surv(PSURV_years,PCENS)~COUNTRY+
              #RSEX+
              BMI+  
              RBILIRUBIN+
              RINR+
...
```{r Supplementary Table 1 recipient}
#Just recipient
...
```{r Supplementary Table 1 recipient and donor}
...
```{r Supplementary table 2 Canada}
#Uni
